...
class CartController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        if(!isset($request->tempuserid)){
            ...
            $owner_ids = Cart::where('temp_user_id', $request->tempuserid)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        }
        ...
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                ...
                if(!isset($request->tempuserid)){
                    ...
                    $shop_items_raw_data = Cart::where('temp_user_id', $request->tempuserid)->where('owner_id', $owner_id)->get()->toArray();
                }
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $shop_items_data_item['rating'] = $product->rating;
        ...
        if(count($shops) > 0){
            return response()->json([
                'status' => true,
                'message' => 'Cart Fatch Successfully',
                'data' => $shops
            ]);
        }else{
            return response()->json([
    ...
    public function destroy(Request $request,$id)
    {
        ...
        if($cart && $cart->delete()){
            return response()->json([
                'result' => true,
                'message' => translate('Product is successfully removed from your cart')
            ], 200);
        }else{
            return response()->json([
                'result' => false,
    ...
    public function allCartDelete(Request $request)
    {
        $cart = '';
        ...
        if(isset($request->tempuserid)){
            ...
            $cart = Cart::where('temp_user_id', $tempuserid)->delete();
        }
        if($cart){
            return response()->json([
                'result' => true,
