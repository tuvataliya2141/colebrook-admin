...
class CategoryUtility
{
    ...
    public static function get_immediate_children_laposies($id, $with_trashed = false, $as_array = false)
    {
        $children = $with_trashed ? Category::where('parent_id', $id)->orderBy('order_level', 'desc')->groupBy('order_by')->get() : Category::where('parent_id', $id)->orderBy('order_level', 'desc')->groupBy('order_by')->get();
        // $children = $with_trashed ? Category::where('parent_id', $id)->orderBy('order_level', 'desc')->get() : Category::where('parent_id', $id)->orderBy('order_level', 'desc')->get();
        $children = $as_array && !is_null($children) ? $children->toArray() : $children;
    ...
    public static function get_immediate_children_ids_laposies($id, $with_trashed = false)
    {

        $children = CategoryUtility::get_immediate_children_laposies($id, $with_trashed, true);
