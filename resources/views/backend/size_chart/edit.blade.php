...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		<div class="col">
			<h1 class="h3">{{ translate('Edit Size Chart') }}</h1>
		</div>
	</div>
</div>
<div class="card">
	<form class="p-4" action="{{ route('size-chart.update') }}" method="POST" enctype="multipart/form-data">
		@csrf
		{{-- <input type="hidden" name="_method" value="PATCH"> --}}
		<input type="hidden" name="size_id" value="{{ $sizeChart->id }}">
		...
		<div class="card-body">
			<div class="form-group row">
				<div class="col-md-6">
					...
					<div class="col-md-10">
						@php $catData = \App\Models\Category::where('name', $sizeChart->name)->first(); @endphp
						<select class="form-control aiz-selectpicker" name="category_id" data-selected="{{ $catData->id }}" id="category_id" data-live-search="true" required>
				...
				<div class="col-md-6">
					...
					<div class="col-md-8">
						<div class="input-group" data-toggle="aizuploader" data-type="image">
							...
							<input type="hidden" name="image" value="{{ $sizeChart->image }}" class="selected-files">
						</div>
			...
			<div class="form-group">
				<table class="table table-bordered table-hover" id="dynamic_field">
					...
					@php $size = json_decode($sizeChart->size_values) @endphp
					@foreach ($size as $key => $val)
						<tr id="row{{ $key }}">
							<td><input type="text" name="size[]" value="{{ $val->size }}" placeholder="Enter the Size like: XL" class="form-control size_list" required/></td>
							<td><input type="text" name="title[]" value="{{ $val->title }}" placeholder="Enter the  Title like: Chest" class="form-control title_list" required/></td>
							<td><input type="text" name="inches_value[]" value="{{ $val->inches_value }}" placeholder="Enter the value in Inches" class="form-control inches_value" required/></td>
							<td><input type="text" name="cm_value[]" value="{{ $val->cm_value }}" placeholder="Enter the value in Centimeter" class="form-control cm_value" required/></td>
